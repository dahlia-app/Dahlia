...
import rest.dahlia.utils.readObject
import rest.dahlia.utils.readString
import java.io.File
import java.util.UUID
import rest.dahlia.domain.dto.Folder as DtoFolder
import rest.dahlia.domain.dto.ProjectConfig as DtoProjectConfig
import rest.dahlia.domain.dto.Request as DtoRequest
import rest.dahlia.domain.dto.Variable as DtoVariable
...
class Project(
    ...
    companion object {
        ...
        fun new(name: String, fileName: String, dir: File): Project {
            val config = ProjectConfig(DtoProjectConfig(name = name))
            val proj = Project(File(dir, "$fileName.dlp"), config)
            proj.save()
            return proj
        }
